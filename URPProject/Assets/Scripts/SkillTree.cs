...
public class SkillNode
{
    ...
    public SkillNode(string id, string name, object skill, Vector2 position, string skillSeries, int skillLevel, List<string> requiredSkills = null)
    {
        Id = id; Name = name;
        Skill = skill;
        Position = position;
...
public class SkillTree              //특성 트리 클래스
{
    ...
    public bool LockSkill(string skillId)
    {
        if(nodeDictionary.TryGetValue(skillId,out SkillNode node))
        {
            ...
            {
                if (otherNode.isUnlocked && otherNode.RequiredSkills.Contains(skillId))
                {
    ...
    public List<SkillNode> GetAllNode()
    {
